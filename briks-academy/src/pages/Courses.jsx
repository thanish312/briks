...
export default function Courses() {
  return (
    <Box component={motion.div} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}
      ...
      <Typography variant="h5" fontWeight={600} color="text.secondary" sx={{ mb: 5 }}>
        Explore a variety of courses designed to help you succeed.
      </Typography>
      <Grid container spacing={5}>
        {courses.map((c, i) => (
          <Grid item xs={12} md={4} key={i}>
            <Card component={motion.div} whileHover={{ scale: 1.04, boxShadow: 8 }} sx={{ borderRadius: 4, boxShadow: 4, height: '100%' }}>
              <CardContent>
                <Typography variant="h6" fontWeight={700} gutterBottom>{c.title}</Typography>
